...
def run_celebs_final():
    ...
    try:
        ...
        if response.status_code != 200:
            print("خطأ في الاتصال بالمصدر")
            return

        soup = BeautifulSoup(response.content, 'xml')
        items = soup.find_all('item')
        
        # شريط الأخبار العلوي - تم استبدال أيقونات الرياضة بأيقونات فنية
        ...
        for i, item in enumerate(items[:24]):
            ...
            # بناء كرت الخبر - هوية بصرية فنية باللون الأرجواني (Purple/Pink)
            news_html += f'''
...
<body>
    ...
    <footer style="text-align:center; padding: 40px; color: #444;">
        ...
        with open("index.html", "w", encoding="utf-8") as f:
            f.write(full_html)
        print("Done: The Celeb Portal is fully updated and the sports content is purged.")
